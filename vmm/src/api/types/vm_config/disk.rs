...
use crate::vm_config;
use crate::vm_config::{DEFAULT_DISK_NUM_QUEUES, DEFAULT_DISK_QUEUE_SIZE};
...
impl From<DiskConfig> for vm_config::DiskConfig {
    fn from(value: DiskConfig) -> Self {
        Self {
            ...
            serial: value.serial,
            queue_affinity: value.queue_affinity.map(|queue_affinity| {
                queue_affinity
                    .into_vec()
                    .into_iter()
                    .map(Into::into)
...
impl From<&vm_config::DiskConfig> for DiskConfig {
    fn from(value: &vm_config::DiskConfig) -> Self {
        Self {
            pci_common: (&value.pci_common).into(),
            ...
            queue_affinity: value.queue_affinity.as_ref().map(|queue_affinity| {
                queue_affinity
                    .iter()
                    .map(Into::into)
                    .collect::<Vec<_>>()
...
impl From<VirtQueueAffinity> for vm_config::VirtQueueAffinity {
    fn from(value: VirtQueueAffinity) -> Self {
        Self {
...
impl From<&vm_config::VirtQueueAffinity> for VirtQueueAffinity {
    fn from(value: &vm_config::VirtQueueAffinity) -> Self {
        Self {
            queue_index: value.queue_index,
...
mod tests {
    ...
    #[test]
    fn test_disk_parsing() -> Result<(), Error> {
        assert_eq!(
