...
use log::warn;
use option_parser::{IntegerList, OptionParser};
...
impl From<GenericVhostUserConfig> for vm_config::GenericVhostUserConfig {
...
impl From<&vm_config::GenericVhostUserConfig> for GenericVhostUserConfig {
    fn from(value: &vm_config::GenericVhostUserConfig) -> Self {
        Self {
            pci_common: (&value.pci_common).into(),
...
impl GenericVhostUserConfig {
    ...
    pub fn parse(vhost_user: &str) -> Result<Self, Error> {
...
id=\"{id}\",pci_segment={pci_segment},queue_sizes={queue_sizes}"
    ...
    #[test]
    fn test_parse_vhost_user() -> Result<(), Error> {
