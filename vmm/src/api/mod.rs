...
use crate::migration::{
    VmReceiveMigrationData, VmReceiveMigrationDataValidationError, VmSendMigrationConfigError,
    VmSendMigrationData,
};
...
use crate::{Error as VmmError, api};
...
impl ApiAction for VmReceiveMigration {
    type RequestBody = api::types::VmReceiveMigrationData;
    type ResponseBody = Option<Body>;
    ...
    fn request(&self, data: Self::RequestBody, response_sender: Sender<ApiResponse>) -> ApiRequest {
        Box::new(move |vmm| {
            ...
            let response = data
                .try_into()
                .map_err(|error: VmReceiveMigrationDataValidationError| {
                    ApiError::VmReceiveMigration(MigratableError::MigrateReceive(error.into()))
                })
                .and_then(|data| {
...
impl ApiAction for VmSendMigration {
    type RequestBody = api::types::VmSendMigrationData;
    type ResponseBody = Option<Body>;
    ...
    fn request(&self, data: Self::RequestBody, response_sender: Sender<ApiResponse>) -> ApiRequest {
        Box::new(move |vmm| {
            ...
            let response = data
                .try_into()
                .map_err(|error: VmSendMigrationConfigError| {
                    ApiError::VmSendMigration(MigratableError::MigrateSend(error.into()))
                })
                .and_then(|data| {
