...
use crate::external_fds::{ExternalFdOperation, ExternalFds, IngestScmRights, UpdateFds};
            ...
            Method::Put => {
                match &req.body {
                    Some(body) => {
                        ...
                        if let Err(error) = vm_config.ingest_scm_rights(Vec::new()) {
                            return error_response(error.into());
                        }

                        if let Err(error) = vm_config
...
impl PutHandler for VmAddDevice {
    fn handle_request(
        ...
        if let Some(body) = body {
            ...
            device_cfg.ingest_scm_rights(files)?;
...
impl PutHandler for VmAddNet {
    fn handle_request(
        ...
        if let Some(body) = body {
            ...
            net_cfg.ingest_scm_rights(files)?;
...
impl PutHandler for VmRestore {
    fn handle_request(
        ...
        if let Some(body) = body {
            ...
            restore_cfg.ingest_scm_rights(files)?;
