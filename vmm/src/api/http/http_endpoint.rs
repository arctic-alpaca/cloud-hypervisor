...
use serializable_fd::{Activatable, FdDevice, Serialized};
            ...
            Method::Put => {
                match &req.body {
                    Some(body) => {
                        ...
                        let vm_config = Box::new(vm_config.activate(&mut HashMap::new()).unwrap());
...
impl PutHandler for VmAddNet {
    fn handle_request(
        ...
        if let Some(body) = body {
            ...
            // TODO(de_ser): error handling
            let mut map = [(
                FdDevice::new_net(net_cfg.pci_common.id.clone().unwrap()),
                files.into_iter().map(|file| file.into()).collect(),
            )]
            .into();
            let net_cfg = net_cfg.activate(&mut map).unwrap();
...
impl PutHandler for VmReceiveMigration {
    fn handle_request(
        ...
        if let Some(body) = body {
            let net_cfg: VmReceiveMigrationData<Serialized> = serde_json::from_slice(body.raw())?;
            ...
            let net_cfg = net_cfg.activate(&mut files).unwrap();
...
impl PutHandler for VmRestore {
    fn handle_request(
        ...
        if let Some(body) = body {
            ...
            let restore_cfg = restore_cfg.activate(&mut files).unwrap();
