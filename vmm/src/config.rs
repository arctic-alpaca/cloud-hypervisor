...
id=\"{id}\",pci_segment={pci_segment},queue_sizes={queue_sizes}"
    ...
    fn test_restore_config_serde() {
        assert_eq!(
            serde_json::from_str::<RestoreConfig<Serialized>>(
    ...
    fn test_config_validation() {
        ...
        let mut still_valid_config = valid_config.clone();
        let fd1 = File::open("/dev/null").unwrap();
        let fd2 = File::open("/dev/null").unwrap();
        still_valid_config.add_preserved_fds(vec![fd1.into(), fd2.into()]);
