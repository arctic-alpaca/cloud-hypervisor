...
use crate::external_fds::{
    ExternalFdOperation, ExternalFdTarget, ExternalFds, FdUpdateError, UpdateFds,
};
...
mod unit_tests {
    ...
    use super::*;
    use crate::external_fds::ExternalFdsEntry;
...
id=\"{id}\",pci_segment={pci_segment},queue_sizes={queue_sizes}"
    ...
    fn test_restore_parsing() -> Result<()> {
        ...
        assert_eq!(
            ...
            RestoreConfig {
                ...
                zone_updates: vec![],
                external_fds: vec![
                    ExternalFdsEntry::new::<RawFd>(net_target("net0"), vec![3, 4],),
                    ExternalFdsEntry::new::<RawFd>(net_target("net1"), vec![5, 6, 7, 8],)
                ]
                .into(),
            }
        );
        ...
        assert_eq!(
            ...
            RestoreConfig {
                ...
                zone_updates: vec![],
                external_fds: vec![
                    ExternalFdsEntry::new::<RawFd>(vfio_target("vfio0"), vec![5],),
                    ExternalFdsEntry::new::<RawFd>(vfio_target("vfio1"), vec![6],),
                    ExternalFdsEntry::new::<RawFd>(ExternalFdTarget::Iommu, vec![7]),
                ]
                .into(),
            }
        );
    ...
    fn test_restore_config_validation() {
        ...
        let valid_config = RestoreConfig {
            ...
            zone_updates: vec![],
            external_fds: vec![
                ExternalFdsEntry::new::<RawFd>(net_target("net0"), vec![3, 4, 5, 6]),
                ExternalFdsEntry::new::<RawFd>(net_target("net1"), vec![7, 8]),
            ]
            .into(),
        };
        ...
        invalid_config.external_fds = vec![ExternalFdsEntry::new::<RawFd>(
            net_target("netx"),
            vec![3, 4, 5, 6],
        )]
        .into();
        assert_eq!(
            ...
            Err(ValidationError::Fd(FdUpdateError::MissingFds(net_target(
                "net0"
            ))))
        );
        ...
        assert_eq!(
            invalid_config.validate(&snapshot_vm_config),
            Err(ValidationError::Fd(FdUpdateError::DuplicatedTargetEntry(
                net_target("net0")
            )))
        );
        ...
        assert_eq!(
            invalid_config.validate(&snapshot_vm_config),
            Err(ValidationError::Fd(FdUpdateError::FdAmountMismatch {
    ...
    fn test_restore_config_vfio_fds_validation() {
        ...
        let valid_config = RestoreConfig {
            ...
            external_fds: vec![
                ExternalFdsEntry::new::<RawFd>(vfio_target("vfio0"), vec![5]),
                ExternalFdsEntry::new::<RawFd>(ExternalFdTarget::Iommu, vec![6]),
            ]
            .into(),
        };
        valid_config.validate(&snapshot_vm_config).unwrap();
        ...
        let missing_iommufd = RestoreConfig {
            external_fds: vec![ExternalFdsEntry::new::<RawFd>(
                vfio_target("vfio0"),
                vec![5],
            )]
            .into(),
        ...
        assert_eq!(
            ...
            Err(ValidationError::Fd(FdUpdateError::VfioFdsWithoutIommuFd))
        );

        let unknown_id = RestoreConfig {
            external_fds: vec![
                ExternalFdsEntry::new::<RawFd>(vfio_target("missing"), vec![5]),
                ExternalFdsEntry::new::<RawFd>(ExternalFdTarget::Iommu, vec![6]),
            ]
            .into(),
            ..valid_config.clone()
        ...
        assert_eq!(
            ...
            Err(ValidationError::Fd(FdUpdateError::SuperfluousFds(vec![
                vfio_target("missing")
            ])))
        );
        ...
        let duplicate_id = RestoreConfig {
            external_fds: vec![
                ExternalFdsEntry::new::<RawFd>(vfio_target("vfio0"), vec![5]),
                ExternalFdsEntry::new::<RawFd>(vfio_target("vfio0"), vec![6]),
                ExternalFdsEntry::new::<RawFd>(ExternalFdTarget::Iommu, vec![6]),
            ]
            .into(),
            ..valid_config.clone()
        ...
        assert_eq!(
            ...
            Err(ValidationError::Fd(FdUpdateError::MissingFds(vfio_target(
                "vfio0"
            ))))
        );
        ...
        assert_eq!(
            ...
            Err(ValidationError::Fd(FdUpdateError::IommufdFdRequiresIommufd))
        );
    }
